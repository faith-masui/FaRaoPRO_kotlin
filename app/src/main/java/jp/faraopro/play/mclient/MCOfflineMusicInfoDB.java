 ...
 * 
 */
public class MCOfflineMusicInfoDB implements IMCMusicInfoDB {

	/**
	 * DB名称
	 */
	public static final String TABLE_NAME = "OfflineMusicInfoData";
	/**
	 * データID
	 */
	 ...
	 * @param database
	 */
	public MCOfflineMusicInfoDB(SQLiteDatabase database) {
		mDatabase = database;
	}
	...
	public int checkTable() {
		...
		String query = "SELECT COUNT(*) FROM sqlite_master WHERE type=" + "'table' AND name='"
				+ MCOfflineMusicInfoDB.TABLE_NAME + "';";
	...
	public IMCMusicItemInfo findOldest(boolean rev) {
		String order = " ASC";
		if (rev)
			order = " DESC";
		IMCMusicItemInfo info = null;
		Cursor cursor = mDatabase.query(TABLE_NAME, COLUMNS, null, null, null, null, COLUM_REG_TIME + order);
		if (cursor != null) {
			if (cursor.moveToFirst()) {
				info = setInfoFromCursor(cursor);
			}
	...
	public void deleteAll() {
		mDatabase.execSQL(MCOfflineMusicInfoDBHelper.DROP_TABLE_SQL);
		mDatabase.execSQL(MCOfflineMusicInfoDBHelper.CREATE_TABLE_SQL);
	}

	@Override
	public int getSize() {
		int value = 0;
		IMCMusicItemList list = findAll();
		if (list != null) {
			value = list.getSize();
			list.clear();
			list = null;
